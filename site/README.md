...
- 새 저장소에 현재 폴더 내용만 올렸다면 Root directory는 저장소 루트 그대로 사용하면 됩니다.
- 현재 공개 GitHub 저장소처럼 루트에 `하별 홈페이지 제작` 폴더가 한 번 더 있는 구조라면 Root directory를 그 폴더로 지정해야 합니다.
- 별도 빌드 산출물 폴더가 없으므로, 선택한 Root directory 자체가 배포 대상입니다.
